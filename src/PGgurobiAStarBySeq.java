...
//	private EMatrixEntryWIndex pairwiseMinEnergyMatrix [][] = null;
	 ...
	 * 		reduced size
	 */
	PGgurobiAStarBySeq (int treeLevels, int numRotForRes[], Emat arpMatrix, double upperE,boolean reorder){
...
//					double finalScore = gOpt.getObjVal();
					...
					ToulbarOptimization optimizer = new ToulbarOptimization(expNode,emat,seqIndicesPerLevel,numRotRemainingBySeq,
...
//									nextLevelNodes[node[0].nodeNum] = node[0];
								...
								nextLevelNodes[rot].fScore = toulbarFscore(nextLevelNodes[rot]);
...
//	}
	...
	private double toulbarFscore(PGQueueNode node) {
		ToulbarOptimization optimizer = new ToulbarOptimization(node,emat,seqIndicesPerLevel,numRotRemainingBySeq,numNodesForLevel,upperE);
		System.out.print(".");
