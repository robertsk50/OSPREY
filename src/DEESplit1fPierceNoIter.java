 ...
 * 
 */
public class DEESplit1fPierceNoIter {
...
//	double pairIntMinDEE[] = null;
	...
	//constructor
	DEESplit1fPierceNoIter(Emat arpMatrix, double initEw, boolean[] residueMut,
...
//			System.out.println("Current run: "+numRuns);
			...
			for (int curPos=0; curPos<pairwiseMinEnergyMatrix.numMutPos(); curPos++){ //For each position i

				if(distrDEE && !resInMut[curPos]) //Skip if distrDEE and not current position
					continue;

				int singleCtr = -1;
				
				for(int i_r_aa=0;i_r_aa < pairwiseMinEnergyMatrix.singles.E[curPos].length;i_r_aa++){
					for(int i_r_rot=0;i_r_rot < pairwiseMinEnergyMatrix.singles.E[curPos][i_r_aa].length;i_r_rot++){
				
					singleCtr++;
					if (pairwiseMinEnergyMatrix.singles.pruned[curPos][i_r_aa][i_r_rot])//Skip if pruned
						continue;
					...
					setupDiffMatrix(curPos,i_r_aa,i_r_rot);

					if (CanEliminate(curPos,i_r_aa,i_r_rot)){
						pairwiseMinEnergyMatrix.singles.pruned[curPos][i_r_aa][i_r_rot] = true;
						prunedCurRun++;
					}
...
//			System.out.println("Split1f RotPruned:"+prunedCurRun);
			...
			for (int j=0; j<pairwiseMinEnergyMatrix.numMutPos(); j++){//For all other positions j, j!=i 
				...
				for(int j_u_aa=0;j_u_aa < pairwiseMinEnergyMatrix.singles.E[j].length;j_u_aa++){
					...
					if(pairwiseMinEnergyMatrix.getPairPruned(i,i_r_aa,i_r_rot,j,j_u_aa,j_u_rot)) //skip if i_r,j_u pruned
						continue;
					//skip if i_t,j_u pruned, then i_r still might be better than it
	 ...
	 * 
	 */
	private boolean CanEliminate (int i,int i_r_aa,int i_r_rot){

		double checkSum = 0;

		
		//For each splitting position k
		for (int k=0; k<pairwiseMinEnergyMatrix.numMutPos(); k++){ //for each splitting position

			if(k == i || !pairwiseMinEnergyMatrix.areNeighbors(i,k)) //For each split pos not equal to curPos
				continue;
			
			boolean partitionPruned[] = new boolean[numRotForRes[k]]; //prune r at i for each partition
			...
			for(int i_t_aa=0;i_t_aa < pairwiseMinEnergyMatrix.singles.E[i].length;i_t_aa++){
				...
				for (int j=0; j<pairwiseMinEnergyMatrix.numMutPos(); j++){//for all other positions j, j!=i, j!=k 
					if(j == k || j == i || !pairwiseMinEnergyMatrix.areNeighbors(i, j))
						continue;
				...
				for(int k_v_aa=0;k_v_aa < pairwiseMinEnergyMatrix.singles.E[k].length;k_v_aa++){
					...
					double splitDiff = pairwiseMinEnergyMatrix.pairs.E[i][i_r_aa][i_r_rot][k][k_v_aa][k_v_rot] - 
											pairwiseMinEnergyMatrix.pairs.E[i][i_t_aa][i_t_rot][k][k_v_aa][k_v_rot]; 
					double totalE = checkSum + splitDiff;
					if( (totalE) > curEw)
						partitionPruned[k_v_ctr] = true;
					/*if(pairwiseMinEnergyMatrix.getPairPruned(i_r.index, k_v.index))
						partitionPruned[k_v_ctr] = true;*/
			...
			for(int part=0; part<partitionPruned.length;part++)
				if(!partitionPruned[part]){
					allPruned = false;
					break;
				}
