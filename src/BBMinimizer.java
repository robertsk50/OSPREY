...
public class BBMinimizer implements Serializable {
	...
	public void initialize(Molecule mol, Amber96ext theA96ff, MutableResParams strMut, int numStrands) {
	...
	public void minimizeFull(boolean pemComp){
		...
		for (int i=0; i<numStepsFiPsi; i++) {
			...
			for (int j=0; j<strandMut.allMut.length; j++){ //apply the current phi/psi step for each of the flexible residues
				int asResNum = strandMut.allMut[j]; //strandMut is strandResidue specific
				Residue asRes = m.residue[asResNum];
				int str = asRes.strandNumber;
				int strResNum = asRes.strandResidueNumber;
				for (int a=0; a<2; a++){ // (a==0) applies a phi change; (a==1) applies a psi change

					if( (!asRes.nterm && !(strResNum==0) && a==0) ||
							(!asRes.cterm && !(strResNum==(m.strand[str].numberOfResidues-1)) && a==1) ){
						double change = compFiPsi(str, strResNum, a,fiPsiStep,bb,pemComp);
						if (Math.abs(change)>0.001){ //smaller changes should not be applies
							bb.applyFiPsi(m,str,asResNum,change,a);
							molCurFiPsiDisp[asRes.moleculeResidueNumber][a] += change;
						}
					}
				}
			}
	...
	private double compFiPsi(int str, int strResNum, int angleType, double fiPsiStep, Backbone bb, boolean pemComp){
		...
		bb.applyFiPsi(m,str, strResNum,fiPsiStep,angleType);	
		secondEnergy = a96ff.calculateTotalEnergy(m.actualCoordinates,-1);	
		if ( (!checkCAallRes(str)) ) //at least one residue moved too far away or unallowed steric for PEMcomp, so make this step impossible
			secondEnergy[0] = Math.pow(10, 38);
		for (int i=0; i<m.strand[str].numberOfResidues; i++)
			restoreCoord(m.strand[str].residue[i].moleculeResidueNumber,storedCoord[m.strand[str].residue[i].moleculeResidueNumber]);
