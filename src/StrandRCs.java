...
public class StrandRCs extends StrandRotamers {
	...
	ResidueConformationLibrary rcl;
	...
	public static void addUnperturbedRCs(Molecule m, int strandNumber){
		...
		for(Residue res: m.strand[strandNumber].residue){//Residue position
			if(res.isMutable){
		...
		public void storeWTRotamers(Molecule m){
			...
			for(int resNum=0;resNum<numberOfResidues;resNum++){//Residue position
				if( m.strand[strandNumber].residue[resNum].isMutable ){
					if( isProtein )
		...
		//(like applyRotamer)
		public boolean applyRC(Molecule m, int resNum, ResidueConformation rc) {


			AARotamerType AAType = rc.rot.aaType; //rl.getAARotamerIndex(m.strand[strandNumber].residue[resNum].name);
			Rotamer rotNum = rc.rot;//RCRots[resNum][AANum][RCNum];
...
//		}
		...
		/**
		 * This function removes RCs from the residue conformation library and will then renumber the residue
		 * conformations, so this should only be called before the emat is set up. 
		 * 
		 * @param toRemove boolean array of residue conformations to remove from rcl (global rcl indexing)
		 */
		public void removeRCs(boolean[] toRemove, Molecule m){//Remove the RCs with true values in toRemove (indices: position in strand, AA type, RC number)

			//Start from the end so we remove the proper things
			for(int i=toRemove.length-1; i>=0; i--){
				if(toRemove[i]){
					ResidueConformation rc = rcl.allRCs.remove(i);

					Residue res = m.strand[strandNumber].residue[rc.res];
					res.removeRC(rc);
				}	
			}
		...
		public int getTotNumPerturbedRCs(int resNum){//Total number of perturbed RCs at a given residue position
			...
			for(ResidueConformation rc : rcl.allRCs){
				if(rc.res == resNum && rc.pertState != 0)
					count++;
			}
		...
		public ResidueConformationLibrary getRCL(){
			return rcl;
		}
