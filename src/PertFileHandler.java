...
//			}
			...
			while(br.readLine() != null){//Read residue perturbation states and RCs.  Skipping the "RES" line
				...
				if(molResNum != -1){//Don't read perturbation information for residues that aren't in the system
					...
					if( !br.readLine().equalsIgnoreCase("RCs") )
						...
						int numRCsRead = 0;
						RotamerLibrary rl = res.rl;
						int numAATypes = rl.getNumAAallowed();//curStrandRCs.getNumAATypes();
						...
						for(int a=0;a<numRCs;a++){//Read RCs
							...
							if(loadRCs){
								...
								RCInfo[a][0] = rl.getAARotamerIndex(AAName);
	...
	public static void writePertFile(String fileName, Molecule m, Emat emat,
			StrandRotamers strandRot[], MutableResParams strandMut, boolean screenOutput ){
		...
		if(screenOutput){//Check what's pruned
			...
			int flexPosInMol = 0;//Position among flexible residues in the strand


			//Loop through emat to find RCs for each mutable residue
			int ctr=0;
			SinglesIterator iter = emat.singlesIterator();
			while(iter.hasNext()){
				EMatrixEntryWIndex emeWI = iter.next();
				Residue res = m.residue[emat.resByPos.get(emeWI.pos1()).get(0)]; //Useing 0 index assumes no superrotamers
				int globalRC = emat.singles.getRot(emeWI.index)[0];
				ResidueConformation rc = ((StrandRCs)strandRot[res.strandNumber]).rcl.getRC(globalRC);

				if(prunedRCs[emeWI.pos1()] == null){ //Create array if null
					prunedRCs[emeWI.pos1()] = new boolean[emat.numRot(emeWI.pos1())];
					ctr = 0;
				}


				boolean isPruned = emat.getSinglePruned(emeWI.index);
				prunedRCs[res.moleculeResidueNumber][ctr] = isPruned;
				if(!isPruned){
					int resPertState = rc.pertState;
					prunedResPertStates[res.moleculeResidueNumber][resPertState] = false;
